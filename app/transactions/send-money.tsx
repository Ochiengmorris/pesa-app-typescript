...
const SendMoney = () => {
  const { sendMoney, isLoading } = useAuthStore();
  ...
  const handleSendMoney = async () => {
    ...
    setSubmitting(true);
    await sendMoney({ amount: parsedAmount, receiverPhone: contact })
      .then((result) => {
      ...
      <SafeAreaView className="bg-primary h-full">
        <ScrollView>
          <KeyboardAvoidingView
            ...
            style={{ flex: 1 }}
          >
            <Header name={"Send Money"} to={() => router.back()} />
